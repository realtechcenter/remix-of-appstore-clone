  ...
  return (
    <>
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="max-w-xs p-0 overflow-hidden gap-0 [&>div:first-child]:hidden">
          {/* Title in hidden header for accessibility */}
          <DialogHeader className="sr-only">
            <DialogTitle>{language === 'km' ? 'ការ​បង់​ប្រាក់' : 'Payment'}</DialogTitle>
          </DialogHeader>
          {/* macOS title bar with centered title */}
          <div className="flex items-center justify-center px-4 py-2.5 bg-muted/60 border-b border-border/50 relative">
            <DialogClose className="absolute left-4 w-3 h-3 rounded-full bg-[#FF5F57] hover:brightness-90 transition-all group focus:outline-none">
            ...
            <span className="absolute left-[calc(1rem+3.25rem)] flex items-center gap-2">
